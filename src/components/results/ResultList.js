...
const ResultList = ({ userOnly = false }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!isApiAvailable) {
          ...
          setError('Backend API is currently unavailable. Working in offline mode with sample data.');
          setLoading(false);
          return;
        }
  ...
  const handleEditSave = async () => {
    ...
    if (assessment && numScore > assessment.maxScore) {
      setEditModalError(`Score cannot exceed the maximum score of ${assessment.maxScore}`);
      return;
    }
    ...
    try {
      ...
      if (!apiAvailable) {
        ...
        const updatedResults = results.map(r => 
          r.resultId === editResult.resultId 
            ? { ...r, score: numScore } 
            : r
        );
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      if (!apiAvailable) {
        ...
        const filteredResults = results.filter(r => r.resultId !== deleteResult.resultId);
    ...
    <div className="container mt-4">
      ...
      {!apiAvailable && (
        <div className="alert alert-warning mt-3" role="alert">
          <strong>Offline Mode:</strong> Working with local data. Changes will be saved when connection is restored.
          <button className="btn btn-sm btn-outline-warning ms-3" onClick={handleRetry}>
            Retry Connection
          </button>
        </div>
      )}
      ...
      <Modal show={showEditModal} onHide={handleEditModalClose}>
        ...
        <Modal.Body>
          {editModalError && (
            <Alert variant="danger">{editModalError}</Alert>
          )}
          ...
          {editResult && (
            <Form>
              <Form.Group className="mb-3">
                ...
                <Form.Control 
                  type="text" 
                  value={assessments[editResult.assessmentId]?.title || 'Unknown Assessment'} 
                  disabled 
                />
              </Form.Group>
              ...
              <Form.Group className="mb-3">
                ...
                {assessments[editResult.assessmentId] && (
                  <Form.Text className="text-muted">
                    Maximum Score: {assessments[editResult.assessmentId].maxScore}
                  </Form.Text>
                )}
              </Form.Group>
            </Form>
        ...
        <Modal.Body>
          ...
          {deleteResult && assessments[deleteResult.assessmentId] && (
            <div className="alert alert-warning">
              <strong>Assessment:</strong> {assessments[deleteResult.assessmentId].title}<br />
              <strong>Score:</strong> {deleteResult.score} / {assessments[deleteResult.assessmentId].maxScore}<br />
