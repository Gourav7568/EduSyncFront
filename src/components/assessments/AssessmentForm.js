...
const AssessmentForm = () => {
  ...
  const updateOption = (questionIndex, optionIndex, value) => {
    console.log(
      `Updating question ${questionIndex}, option ${optionIndex} to: ${value}`
    );
  ...
  const removeOption = (questionIndex, optionIndex) => {
    console.log(
      `Removing option ${optionIndex} from question ${questionIndex}`
    );
    ...
    if (
      updatedQuestions[questionIndex].correctAnswer === options[optionIndex]
    ) {
  ...
  const calculateTotalPoints = () => {
    return formData.questions.reduce(
      (total, q) => total + (parseInt(q.points) || 0),
      0
    );
  };
  ...
  const validateQuestionsSize = (questions) => {
    ...
    // Database column likely has a limit of around 4000-8000 characters
    // Increased from 1000 to 5000 to accommodate larger assessments
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < formData.questions.length; i++) {
      ...
      if (!q.correctAnswer) {
        setError(
          `Question ${i + 1} has no correct answer. Please specify one.`
        );
        setSubmitting(false);
        return;
      }
      ...
      if (q.options) {
        ...
        if (emptyOptions > 0) {
          setError(
            `Question ${
              i + 1
            } has empty options. Please fill all options or remove them.`
          );
          setSubmitting(false);
          return;
        }
    ...
    try {
      ...
      if (!optimizedValidation.valid) {
        setError(
          "Even after optimization, the questions data is still too large. Please reduce the number or size of questions."
        );
        setSubmitting(false);
        return;
      }
      ...
      console.log(
        "Questions JSON size after optimization:",
        dataToSubmit.questions.length,
        "characters"
      );
      ...
      try {
        ...
        navigate(
          formData.courseId ? `/courses/${formData.courseId}` : "/assessments"
        );
      } catch (apiError) {
        console.error(
          "API Error details:",
